...
temp_times.txt: temp_crime_per_line.txt
	cat temp_crime_per_line.txt | egrep -o 'Time of homicide: [^<]*' | sed 's/Time of homicide: //' | while read x; do date -d "$$x" '+%H'; done > $@
...
final_dat.csv: temp_homicide_final.csv CPDCrimes.csv
	cat CPDCrimes.csv | sed 's/T/ /' | sed 's/-/\//' | sed 's/-/\//' | sed 's/ \([0-9][0-9]\):[0-9][0-9]:[0-9][0-9]\.000/ \1/' > $@
